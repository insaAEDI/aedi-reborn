...
class Entretien {
	...
	public static function GetListeIntervenantByDate($date) {
        return BD::Prepare('SELECT * FROM Entretien_contact WHERE id = (SELECT c.id FROM Entretien_creneau c, Entretien e WHERE c.id_entretien=e.id AND e.date = :date)', array('date' => $date), BD::RECUPERER_TOUT);
    }
